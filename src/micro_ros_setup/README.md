<!-- ─────────────────────────────────────────────────────────────
  ...
  micro_ros_setup (micro-ROS) — 아두이노 Due micro-ROS 빌드 도구

  원본 upstream 커밋: fb214f17e606b838d92b42da9833eced5603a3a6
  ...
  ── 추가한 파일 (업스트림 파일은 하나도 수정 안 함) ──
  · config/COLCON_IGNORE
      config/ 아래 package.xml 12개가 전부 이름이 `firmware` 라서 colcon 이 중복 패키지명으로
      빌드를 멈춘다. 이것들은 create.sh/build.sh 가 크로스컴파일 타깃별 의존성을 선언하는
      템플릿이지 워크스페이스 빌드 대상이 아니다.
      이 패키지가 gitlink(빈 폴더)인 동안 colcon 이 못 보던 것이 내재화로 드러난 것이다.
────────────────────────────────────────────────────────────── -->
