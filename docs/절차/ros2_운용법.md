...
### 🔧 아두이노 RC 배선 진단 3종 (2026-08-13 신설)

**배선이 눈으로 추적이 안 될 때** 쓴다. 전부 **기본 꺼짐**이라 평소 빌드에는 영향이 없다.

```bash
export PATH=$HOME/bin:$PATH   # arduino-cli
```

| 플래그 | 무엇을 보나 | 언제 쓰나 |
|---|---|---|
| `RC_DEBUG=1` | 설정된 3채널의 펄스 폭을 1초마다 | "모드가 대기(0)에서 안 바뀐다" |
| `RC_SCAN=1` | **인터럽트 핀 6개**(2,3,18,19,20,21) 전부 | 3채널 중 하나가 무신호일 때 |
| `PIN_HUNT=1` | **모든 디지털 핀**을 폴링해 RC 펄스를 찾음 | 인터럽트 핀에 다 없을 때 |

```bash
# 예: 전 디지털 핀 사냥
arduino-cli compile --fqbn arduino:avr:mega \
  --build-property compiler.cpp.extra_flags=-DPIN_HUNT=1 arduino/ssf_boat
arduino-cli upload -p /dev/ttyMEGA --fqbn arduino:avr:mega arduino/ssf_boat
```
출력은 시리얼(115200)로 나온다. 읽는 법:
```
HUNT p2=1496us  p3=1548us  p4=1404us      ← 이 세 핀에 RC 신호가 있다
```

🚨 **이 도구가 실제로 잡은 것 (08-13)**
조향선이 **핀 4** 에 꽂혀 있었다. 사진으로도, 기판 납땜 추적으로도 못 찾던 것을
전수 스캔이 한 번에 찾았다. **핀 4 는 인터럽트 핀이 아니라 신호가 와도 코드가 못 읽는다.**

⚠️ **주의**
- `RC_SCAN` 은 같은 핀에 인터럽트를 덮어써서 **정상 RC 경로가 죽는다**(모드가 대기에 머문다). 진단 전용.
- `PIN_HUNT` 는 스윕 한 번에 최대 1.5초 **블로킹**이라 제어 루프가 멈춘다. 진단 전용.
- `PIN_HUNT` 는 **ESC 출력 핀(5~8)을 건너뛴다** — `INPUT` 으로 바꾸면 ESC 신호가 끊기기 때문.
  거기 배선이 있어도 결과에 안 나온다.
- 🚨 **대회 전 반드시 플래그 없이 재컴파일·재업로드할 것.** 진단 줄이 섞이면 브릿지가
  "해석 못한 줄" 경고를 계속 뱉는다.

**업로드는 노트북 직결 포트에서** 하는 게 좋다. 허브를 거치다 업로드가 중간에 끊긴 적이 있고,
그러면 보드가 반쯤 지워진 상태가 된다(운영 중 끊김은 브릿지가 자동 복구하지만 이건 다르다).
...
### 🚨 라이다가 `SL_RESULT_OPERATION_TIMEOUT` 으로 안 뜬다 (2026-09-17)

**직전에 라이다를 강제 종료했기 때문이다.** `Ctrl+C`(SIGINT) 가 아닌 방법으로 죽이면
정지 명령이 안 가서 라이다가 **스캔 상태로 남는다.** 다음 실행이 통째로 실패한다.

```
복구: USB 를 뽑았다 다시 꽂는다
예방: 라이다는 반드시 Ctrl+C 로 끈다
```

### 🚨 브릿지를 다시 띄우면 배가 20초 멈춘다 (2026-09-17)

`ssf_bridge` 가 `/dev/ttyMEGA` 를 여는 순간 **DTR 이 걸려 아두이노가 리셋된다.**
스러스터가 중립으로 가고 **ESC 재시동에 15~25초** 걸린다.

- 브릿지는 **한 번만** 띄운다. 경기 중 재시작 금지
- 출발 전 ESC 시동을 확인한 뒤에는 건드리지 않는다

### 🚨 아두이노를 허브에 꽂지 마라 (2026-09-17)

수신기(FS-iA6B)가 아두이노 5V 로 돈다. 버스전원 허브 경유 시 수신기 전압 **4.09V**(최소 4.0V).
**동작은 하고 경보만 뜬다** — 놓치기 쉽다. 노트북 직결이면 4.70V.
