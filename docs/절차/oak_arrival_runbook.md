# OAK-1 PoE 도착일 런북 (표준 시야각 · Fixed Focus — 2026-09-15 구매모델 확정, W 아님)
...
OAK-1 **PoE** 모델(리얼리스토어 no=45, SJ2096POE)은 USB 가 아니라 **이더넷**이다.

1. PoE 인젝터(또는 PoE 스위치) → 카메라. **카메라 쪽 커넥터 = 표준 RJ45**(데이터시트: "On the device end standard RJ45 cable connector") — M12 아님. 동봉 2m Cat5e 그대로.
   · 전원: **802.3af Class 3 active**(VPOE 37~57V, 최대 0.35A). **USB-C 는 시판 밀봉품 외부에 없음** — 오픈보드(README)에만 있다. PoE 인젝터 필수.
   · 소비전력 **4~5W 전형 / 7.5W 최대** (idle 2.5W). 인젝터 입력(4S)에서 ≈0.4~0.6A.
   · 외형 81.9×81.9×31mm · **IP67** · 장착: 바닥 **¼-20 삼각대 나사** + 뒷면 **M4 ×4**(가로 4.5cm / 세로 2×3.75cm)
   · ⚠️ 동작 주변온도 **0~50°C** — 여름 직사광 아래 검정 IP67 통은 50°C 넘을 수 있다. 그늘/통풍·온도 로깅
...
구매 모델은 **OAK-1 PoE 표준 시야각**이다(W 광각 아님). 공식 데이터시트(Fixed Focus 50cm~∞, IMX378):
**HFOV 69° / VFOV 55° / DFOV 81°**. RealSense 실측 80.32° 보다 좁고, 작년 설정 71.5° 에 가깝다.
→ 광각(120~150°)이었다면 픽셀당 각도가 크게 달라 튜닝이 통째로 무효였겠지만, 69° 는 **재튜닝 폭이
작고 원거리 부표 해상도가 좋다.** 그래도 아래 절차로 **반드시 실측**한다 — 해상도·크롭에 따라 다르다.

### 실측 절차

카탈로그의 DFOV(82°)를 그대로 넣지 마라 — 우리가 쓰는 건 **HFOV(수평)** 이고, 해상도·크롭에 따라 다르다.
스펙 HFOV 69° 는 **검산 기준값**이다: 실측이 69° 에서 크게 벗어나면 D 측정이나 크롭 설정을 의심하라.

2m 에서 필요한 좌우폭 ≈ **2.75m**(69°) — 실내에서 충분히 가능하다. 카메라 **렌즈면**(몸체 앞면 아님)
기준으로 자로 `D` 를 정확히 잰다.
...
#### 📌 이 검산이 실제로 걸러낸다 — RealSense 로 미리 해봤다 (2026-08-07)

현행 D455 로 이 절차를 그대로 돌려봤더니 **불일치가 나왔다:**

| 출처 | 값 |
|---|---|
| `camera_info` 실측 | `fx=387.36`, `fy=386.82`, `cx=321.60`, `cy=239.32` (640x480) → **HFOV 79.12°** |
| `vision.yaml` 설정 | `hfov_deg: 71.5` |
| **차이** | **+7.62°** |

`hfov_deg: 71.5` 는 스펙시트가 아니라 *"0.5m 에서 80px = 9cm"* 벤치 캘리브레이션의 **역산**이다
(CLAUDE.md 3-3 V3). 그 역산이 카메라가 직접 보고하는 내부 파라미터와 어긋난다.

영향: `angle = -atan((vX-cx)/fx)` 에서 **화면 가장자리 약 3.8° 오차**.
`align_tol_deg` 가 5° 이므로 무시할 수 없다.

⚠️ **아직 확정이 아니다.** USB 케이블 문제로 프레임이 4장만 온 상태에서 읽은 값이다
(자세한 경위는 `docs/결과분석/실기통합시험_LiDAR_IMU_20260806.md`).
**카메라가 정상 스트리밍되는 상태에서 다시 읽어 확정할 것.**

> **얻은 교훈:** 이 검산 단계는 형식적인 절차가 아니다. **실제로 불일치를 잡아낸다.**
> OAK 도착일에 절대 건너뛰지 말 것.

#### ⏸ 검토 항목 — `hfov_deg` 를 없애고 `camera_info` 를 직접 읽을까

위 불일치는 "사람이 재서 yaml 에 넣은 값이 틀어져 있었다" 는 사례다.
CLAUDE.md 3-3 V3 은 **해상도**에 대해 이미 같은 판단을 내렸다:

> **해상도는 파라미터가 아니다** — 매 프레임 실제 이미지에서 읽는다. 파라미터로 두면
> yaml 을 안 고친 채 해상도만 바꿨을 때 조용히 각도가 틀어진다.

같은 논리를 `fx` 에도 적용하면 — `camera_info` 를 구독해 `fx`·`cx` 를 그대로 쓰면 —
**카메라를 바꿔도 yaml 을 안 고쳐도 된다.** OAK 도착일 작업이 "화각 실측 + yaml 수정" 에서
**"토픽 이름만 바꾸기"** 로 줄어든다.

🚨 **그런데 지금 하지 않는다.** 근거 없이 못 바꾸는 이유:
1. **OAK 가 올바른 `camera_info` 를 낼지 모른다.** `depthai-ros` 가 공장 캘리브레이션을
   내보내는지, 우리가 캘리브레이션을 해야 하는지 **실물 없이 확정 못 한다.**
2. **광각 왜곡은 그래도 남는다.** `fx` 를 맞춰도 아래 §"광각은 핀홀 모델이 깨진다" 는 유효하다.
   120~150° 에서는 이쪽이 더 크다.
3. **비전 4종은 원자적**이다(CLAUDE.md 1-1 예외). 쪼개서 못 고친다.

→ **OAK 도착일에 `camera_info` 를 실제로 확인한 뒤 판단한다.**
   멀쩡한 값이 나오면 그때 바꾸는 게 싸다. 지금 바꾸면 검증할 카메라가 없다.
