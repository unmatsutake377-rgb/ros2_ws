...
- 업로드: Arduino IDE → 보드 **"Arduino Mega or Mega 2560"** → USB 연결 → 업로드
- **[2026-07-25] micro-ROS 폐기 → 시리얼 브릿지 방식** (회로팀 브릿지 노드 수용). micro_ros_arduino 라이브러리·에이전트·platform.txt 패치 전부 불필요해짐
- **[2026-08-10 정정] 포트는 하나다.** 이전 판은 "Programming 포트 = 명령 / Native 포트 = 상태"
  라고 적혀 있었는데 그건 **Due 시절 얘기**다. Mega 2560 은 USB 시리얼이 하나뿐이고,
  펌웨어도 `Serial` 하나로 **양방향**을 쓴다:
  - 받기: `L<좌µs>,R<우µs>\n` (브릿지 → Mega)
  - 보내기: `S,mode,watchdog,boatId,estop,FL,FR,RL,RR` 10Hz (Mega → 브릿지)
  같은 선으로 오가므로 **브릿지가 상태 줄을 읽어서 비워줘야 한다** — 안 읽으면 입력 버퍼가 쌓인다.
- **브릿지는 `src/ssf_bridge/` 에 있다** (2026-08-10 신설, 작년 발굴 코드를 되살림).
  상태를 `/boat_mode`·`/boat_cmd_watchdog`·`/boat_estop`·`/boat_id` 로 올린다.
  아두이노 없이 검증: `python3 tools/fake_mega.py`
- 노트북측 `Motor_run` 토픽 계약은 무변경 — 브릿지가 번역 담당. 프로토콜 상세는 설계문서 §1-2
- 배 A/B 구분은 코드가 아니라 **배에 달린 ID 핀(DIP 스위치)** 으로 자동 인식 — 펌웨어는 두 배 공용 1벌

## ~~⚠️ 새 컴퓨터에서 컴파일 시 필수 패치~~ — **더 이상 필요 없다 (보관용)**

> **[2026-08-10]** 아래는 **Due + micro-ROS 시절** 절차다. 보드가 Mega 2560 으로 바뀌고
> micro-ROS 를 폐기하면서 **전부 불필요해졌다**(위 본문에도 그렇게 적혀 있는데 이 절이 남아
> 모순이었다). Mega 에서 이 패치를 찾아 헤매지 말 것. 되돌릴 일이 생길 때를 위해 지운다.

<details>
<summary>(옛 절차 펼치기)</summary>

### 새 컴퓨터에서 컴파일 시 필수 패치 (undefined reference 에러 대책)
